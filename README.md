...
- 标题在已获取且番号匹配的资料中优先选择中文候选，繁体转简体；没有时先读成功译文缓存，再尝试 Google 和可选的 OpenAI Responses 兼容备用翻译，不改变封面来源，也不额外遍历网站。中文候选按含汉字且不含假名判断，纯汉字日文与中文仍可能无法区分。
...
在 `.env` 中同时配置 `TITLE_TRANSLATION_BASE_URL`（以 `/v1` 结尾）、`TITLE_TRANSLATION_API_KEY` 和 `TITLE_TRANSLATION_MODEL`，即可启用 OpenAI Responses 兼容的备用翻译。只在主翻译失败时调用；服务须支持非流式 `/responses`、`reasoning.effort=low`。请求最多等待 45 秒，只发送待译标题及作品资料中的姓名，不发送 Telegram 凭据。未配置时仍可使用 Google 与已有缓存。
